...
| `TOMSHLEY_CICD_FLOW_MESSAGE_PREFIX_PATTERN` | No | (unset) | POSIX extended regex used to **derive** the prefix when `FLOW_MESSAGE_PREFIX` is unset: recent commit subjects on the checked-out branch are scanned and the first match becomes the prefix. Useful to propagate an issue-tracker key already present in the branch history (e.g. `PROJ-[0-9]+` or `#[0-9]+`) into flow-generated messages without hardcoding a key — for example to satisfy a commit-message verification rule. No match ⇒ no prefix (with a warning). |
